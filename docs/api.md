...
| `sort`                          | `date-desc` (ברירת מחדל), `date-asc`, `duration-asc`, `duration-desc`, `title-asc`, `title-desc`, `relevance` | עם `q` ברירת המחדל היא רלוונטיות |
...
תגית משותפת 2 (עד שלוש), אותו ערוץ 1. שוויון נשבר לפי תאריך הוספה.
המשקולות מגיעות מ-`shared/core/relevance.ts` ומוזרקות ל-SQL, כך ששני
העותקים לא יכולים להיפרד.
